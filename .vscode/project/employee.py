...
class employeeClass:
    def __init__(self, root):
        ...
        lbl_passwd=Label(self.root,text="Password",font=("goudy old style",15),bg="white").place(x=350,y=230)
        ...
        txt_passwd=Entry(self.root,textvariable=self.var_passwd,font=("goudy old style",15),bg="lightyellow").place(x=500,y=230,width=180)
        ...
        self.txt_address.place(x=100,y=270,width=300,height=60)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
               ...
               row=cur.fetchone()
               if row!=None:
                   messagebox.showerror("Error","This user already exists, try different value",parent=self.root)
               else:
               ...
               if row==None:
                   messagebox.showerror("Error","Invalid Employee ID",parent=self.root)
                    ...
                    if op== True:
                        ...
                        con.commit()
                        messagebox.showinfo("Delete","Employee Delete Successfully",parent=self.root)
                        self.clear()
        ...
        try:
            if self.var_searchby.get()=="Select":
                ...
                if len(rows)!=0:
                    ...
                    messagebox.showerror("Error","No required found!!!",parent=self.root)
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
