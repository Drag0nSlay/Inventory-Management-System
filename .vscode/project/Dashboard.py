...
from app_paths import BILL_DIR, DB_PATH, asset_path, script_path
from inventory_utils import inventory_counts
...
class IMS:
    def __init__(self, root):
        ...
        self.icon_title = PhotoImage(file=asset_path("logo1.png"))
        title = Label(
            self.root,
        ...
        self.menu_logo = PhotoImage(file=asset_path("menuLogo.png"))
        ...
        self.icon_side = PhotoImage(file=asset_path("side.png")).subsample(15)
        ...
        self.lbl_low_stock = Label(self.root, text="Low / Out of Stock\n[ 0 / 0 ]", bd=5, relief=RIDGE, bg="#8e44ad", fg="white", font=("goudy old style", 20, "bold"))
        self.lbl_low_stock.place(x=1000, y=300, width=300, height=150)
    ...
    def launch_script(self, script_name):
        os.system(f'"{sys.executable}" "{script_path(script_name)}"')

    def update_content(self):
        con = sqlite3.connect(database=DB_PATH)
        cur = con.cursor()
        try:
            cur.execute("select qty from product")
            product_rows = [{"qty": row[0]} for row in cur.fetchall()]
            counts = inventory_counts(product_rows)
            self.lbl_product.config(text=f'Total Product\n[ {counts["total"]} ]')
            self.lbl_low_stock.config(text=f'Low / Out of Stock\n[ {counts["low_stock"]} / {counts["out_of_stock"]} ]')
            ...
            bill_count = len(os.listdir(BILL_DIR)) if BILL_DIR.exists() else 0
