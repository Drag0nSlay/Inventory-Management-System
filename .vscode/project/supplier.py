...
#========================================================================================================
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
               ...
               row=cur.fetchone()
               if row!=None:
                   messagebox.showerror("Error","Invoice no. already exists, try different value",parent=self.root)
               else:
    ...
    # Check if row has enough elements before setting var_contact
          if len(row) >= 4:
              self.var_contact.set(row[2])
              self.text_desc.delete('1.0',END)
              self.text_desc.insert(END,row[3])
          else:
             print("Row doesn't have enough elements")  # Add this line for debug
